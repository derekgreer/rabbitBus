...
namespace RabbitBus.Configuration
{
	public interface IConsumeConfigurationContext
	{
		...
		IConsumeConfigurationContext WithDeadLetterExchange(string exchangeName);
		IConsumeConfigurationContext WithDeadLetterExchange(string exchangeName, string routingKey);
	}
}